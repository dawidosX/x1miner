...
std::string sha256_hex(const std::string& data, std::size_t prefix_bytes) {
    ...
    if (rem) std::copy(bytes + offset, bytes + offset + rem, block);
    block[rem] = 0x80;
    ...
    if (prefix_bytes > 32) prefix_bytes = 32;

    std::ostringstream oss;
    oss << std::hex << std::setfill('0');
    for (std::size_t i = 0; i < prefix_bytes; ++i) {
        const uint32_t word = state[i / 4];
        const uint8_t b = static_cast<uint8_t>((word >> (24 - 8 * (i % 4))) & 0xff);

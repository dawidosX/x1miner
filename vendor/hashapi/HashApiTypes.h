...
struct HashApiRequest {
    ...
    int hash_patch = 0;
    int prepare_patch = 1;
    /// 0 = full wave. 1 = create (keygen+first-block). 2 = collect hash (finish+snapshot).
    /// 3 = launch hash (enqueue oneshot, do not wait).
    int wave_role = 0;
    /// GPU arena to allocate (jobs). 0 = use batch_size. Must be >= batch_size.
    std::size_t arena_batch_size = 0;
};
